...
public class GenerarReporte extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try (OutputStream out = response.getOutputStream();
            ...
            Paragraph titulo = new Paragraph(("REPORTE DE " + tipo).toUpperCase(), tituloFont);
            ...
            switch (tipo) {
                case "productos":
                    ...
                    try (Statement st = con.createStatement();
                         ResultSet rs = st.executeQuery("SELECT nombre, stock_actual FROM Productos ORDER BY id_producto DESC")) {
                        while (rs.next()) {
                            tabla.addCell(new PdfPCell(new Phrase(rs.getString("nombre"), bodyFont)));
                            tabla.addCell(new PdfPCell(new Phrase(String.valueOf(rs.getInt("stock_actual")), bodyFont)));
                        }
                    }
                    break;
                ...
                case "produccion":
                    ...
                    try (Statement st = con.createStatement();
                         ResultSet rs = st.executeQuery(
                             "SELECT p.id_produccion, pr.nombre AS producto, p.cantidad_producida " +
                             "FROM Produccion p INNER JOIN Productos pr ON p.id_producto = pr.id_producto " +
                             "ORDER BY p.id_produccion DESC")) {
                        while (rs.next()) {
                            tabla.addCell(new PdfPCell(new Phrase(String.valueOf(rs.getInt("id_produccion")), bodyFont)));
                            tabla.addCell(new PdfPCell(new Phrase(rs.getString("producto"), bodyFont)));
                            tabla.addCell(new PdfPCell(new Phrase(String.valueOf(rs.getInt("cantidad_producida")), bodyFont)));
                        }
                    }
                    break;
                ...
                case "ventas":
                    ...
                    try (Statement st = con.createStatement();
                         ResultSet rs = st.executeQuery("SELECT id_venta, fecha_hora, total FROM Ventas ORDER BY id_venta DESC")) {
                        while (rs.next()) {
                            tabla.addCell(new PdfPCell(new Phrase(String.valueOf(rs.getInt("id_venta")), bodyFont)));
                            tabla.addCell(new PdfPCell(new Phrase(String.valueOf(rs.getTimestamp("fecha_hora")), bodyFont)));
                ...
                case "detalle_venta":
                    ...
                    try (Statement st = con.createStatement();
                         ResultSet rs = st.executeQuery(
                             "SELECT dv.id_detalle, pr.nombre AS producto, dv.cantidad, dv.subtotal_linea " +
